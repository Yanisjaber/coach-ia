...
window.detectRideSequences = function (watts, ftp) {
  try {
    ...
    while (segs.length > 1 && guard++ < 5000) {
      var bi = -1, bd = 1e9;
      for (var q = 0; q < segs.length; q++) { var mn = segs[q].z >= 5 ? 10 : (segs[q].z === 4 ? 25 : 45); if (segs[q].dur < mn && segs[q].dur < bd) { bd = segs[q].dur; bi = q; } }
      if (bi === -1) break;
    ...
    blocks = blocks.filter(function (bl) { var z = zp(bl.work.int), d = bl._durS || ((+bl.work.min || 0) * 60), mn = z >= 5 ? 10 : (z === 4 ? 25 : 45); return d >= mn; });
