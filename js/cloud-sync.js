    ...
    if (data) {
      const arr = data
        ...
        .map(r => ({
          ...
          estWatts: r.est_watts ?? null, estBpm: r.est_bpm ?? null, estSpeed: r.est_speed ?? null,
...
export async function pushTemplate(tpl) {
  ...
  try {
    const row = {
      ...
      est_watts: tpl.estWatts ?? null, est_bpm: tpl.estBpm ?? null, est_speed: tpl.estSpeed ?? null,
  ...
  try {
    if (mode === 'prevu') {
      const row = {
        ...
        est_watts: training.estWatts ?? null, est_bpm: training.estBpm ?? null, est_speed: training.estSpeed ?? null,
        est_kj: training.estKj ?? null, est_if: training.estIf ?? null,
