...
async function pullAllFromCloud() {
  ...
  try {
    ...
    const prevu = (data || []).map(r => ({
      ...
      structure: r.structure ?? null,
      estWatts: r.est_watts ?? null, estBpm: r.est_bpm ?? null, estPace: r.est_pace ?? null,
      estKj: r.est_kj ?? null, estIf: r.est_if ?? null, estSpeed: r.est_speed ?? null,
  ...
  try {
    if (mode === 'prevu') {
      const row = {
        ...
        est_watts: training.estWatts ?? null, est_bpm: training.estBpm ?? null, est_pace: training.estPace ?? null,
        est_kj: training.estKj ?? null, est_if: training.estIf ?? null, est_speed: training.estSpeed ?? null,
